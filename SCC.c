...
void SCC_update(Stack* stack,SCC *scc,NodeIndex* index)
{
	int comp_count, prime_lowlink, temp_node,temp_lowlink;
	temp_node = check_stack_tarjan(stack);
	prime_lowlink = index->index_array[temp_node].lowlink;
	...
	while (temp_lowlink == prime_lowlink) {
		...
		if (scc->components[comp_count].included_nodes_count == scc->components[comp_count].array_size)
		{
			...
			scc->components[comp_count].included_node_ids = realloc(scc->components[comp_count].included_node_ids, scc->components[comp_count].array_size);
		}
		scc->components[comp_count].component_id = comp_count;
		scc->components[comp_count].included_nodes_count++;
		if (scc->components[comp_count].included_node_ids == NULL)
...
int expand_tarjan(int node, NodeIndex* index, Buffer* buffer,SCC* scc)
{
	int expandedNodes = 0;
	...
	while ((temp_node = check_stack_tarjan(stack)) != -1) 
	{
		...
		{	
			do {
				i = 0;
				while (i < buffer->buffer_array[index->index_array[temp_node].offset].emptyNeighborPos) {
					temp_neigh = buffer->buffer_array[index->index_array[temp_node].offset].neighbor[i];
					if (index->index_array[temp_neigh].tarjanvisited != 1) {
						push_stack_tarjan(temp_neigh, stack);
						index->index_array[temp_neigh].tarjanvisited = 1;
						break;
					}
				}
				offset = buffer->buffer_array[index->index_array[temp_node].offset].nextListNode;
			} while (offset != -1);
		}
	}
	/*while (nextNode != -1)
...
void tarjan(NodeIndex* index, NodeIndex* index2, Buffer* buffer, Buffer* buffer2, SCC* scc)
{
	...
	for (i = 0; i < index->size; i++)
	{
		...
		else if (offset != -1)
		{
			...
			expand_tarjan(i, frontier, index, buffer,scc);
